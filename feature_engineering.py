...
# Import second-order Greeks for Vanna levels
try:
    from second_order_greeks import OptionsFeatureEngineering
    VANNA_AVAILABLE = True
except ImportError:
    VANNA_AVAILABLE = False
    print("[WARNING] second_order_greeks module not found - Vanna levels disabled")
...
class FeatureEngineering:
    ...
    def add_vanna_levels(self, dte=30, iv_estimate=0.25, strike_width=5.0):
        """
        Add Vanna-based support and resistance levels using options Greeks.

        Args:
            dte: Days to expiration for options calculation
            iv_estimate: Estimated implied volatility (0.25 = 25%)
            strike_width: Price increment for strike levels
        """
        if not VANNA_AVAILABLE:
            print("[SKIP] Vanna levels not available - install second_order_greeks")
            return self
        ...
        ofe.add_second_order_greeks(dte=dte, iv_estimate=iv_estimate)

        # Calculate Vanna support/resistance
        ofe.add_vanna_support_resistance(strike_width=strike_width, vanna_threshold=0.01)

        # Extract Vanna levels and add to our data
        vanna_cols = [col for col in ofe.df.columns if 'vanna_' in col]
        for col in vanna_cols:
            df[col] = ofe.df[col]

        # Calculate derived features for ML
        if 'vanna_support_1' in df.columns and 'vanna_resistance_1' in df.columns:
            # Distance to nearest levels (as percentage)
            df['dist_to_vanna_support'] = ((df['close'] - df['vanna_support_1']) / df['close']) * 100
            df['dist_to_vanna_resistance'] = ((df['vanna_resistance_1'] - df['close']) / df['close']) * 100

            # Near a Vanna level (within 0.5%)
            df['near_vanna_support'] = (df['dist_to_vanna_support'].abs() < 0.5).astype(int)
            df['near_vanna_resistance'] = (df['dist_to_vanna_resistance'].abs() < 0.5).astype(int)

            # Strength ratio (support vs resistance)
            if 'vanna_support_1_strength' in df.columns and 'vanna_resistance_1_strength' in df.columns:
                df['vanna_strength_ratio'] = df['vanna_support_1_strength'] / (df['vanna_resistance_1_strength'] + 0.0001)
...
if __name__ == "__main__":
    ...
    try:
        ...
        fe.add_vanna_levels(dte=30, iv_estimate=0.25, strike_width=5.0)
        print("[OK] Vanna support/resistance levels")
        ...
        print("\n[OK] Saved to: spy_ml_dataset.csv")
